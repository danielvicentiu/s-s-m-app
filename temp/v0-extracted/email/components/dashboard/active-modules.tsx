          ...
          >
            <div
              className={`flex h-9 w-9 items-center justify-center rounded-lg ${
                mod.status === "active" ? "bg-primary/10" : "bg-amber-100"
              }`}
            >
              <mod.icon
                className={`h-4 w-4 ${
                  mod.status === "active" ? "text-primary" : "text-amber-600"
                }`}
              />
            </div>
            ...
            <span
              className={`inline-flex items-center gap-1 text-[10px] font-medium ${
                mod.status === "active" ? "text-green-600" : "text-amber-600"
              }`}
            >
              {mod.status === "active" ? (
                <>
                  <Check className="h-3 w-3" /> Activ
                </>
              ) : (
                <>
